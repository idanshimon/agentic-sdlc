...
class ProviderUnavailable(Exception):
    """A configured provider remained unavailable; production must fail closed."""

    def __init__(self, stage: str, agent: str, model: str, cause: Exception) -> None:
        self.stage = stage
        self.agent = agent
        self.model = model
        self.cause = cause
        super().__init__(f"provider unavailable for {agent}/{stage} on {model}: {cause}")
    ...
    *,
    ...
    try:
        ...
        if profile not in {"demo", "test", "development"}:
            ...
            raise ProviderUnavailable(stage_key, agent_name, model, exc) from exc
        _logger.warning(
            "Provider call failed (%s/%s on %s): %s — explicit %s stub",
            agent_name, stage_key, model, exc, profile,
        )
        ...
        run.contains_synthetic_output = True
        if stage_key not in run.synthetic_stages:
            run.synthetic_stages.append(stage_key)
    ...
    return CallResult(
        text=text, prompt_tokens=p_tok, completion_tokens=c_tok, usd=usd,
        synthetic=synthetic, error_category=error_category,
    )
...
# --- 7. DELIVER ---------------------------------------------------------------
def _delivery_files(run: RunState) -> list[dict[str, str]]:
    """Project the latest stage artifacts into their durable GitHub file paths."""
    app_code = ""
    test_code = ""
    test_plan = ""
    architecture = ""
    for event in run.events:
        ...
        if payload.get("test_plan"):
            test_plan = str(payload["test_plan"])
        if payload.get("app_code"):
            app_code = str(payload["app_code"])
        elif payload.get("code"):
            app_code = str(payload["code"])
        if payload.get("test_code"):
            test_code = str(payload["test_code"])

    files = [
        {"path": "src/main.py", "content": app_code or "# generated by agentic-sdlc\n"},
        {"path": "docs/architecture.md", "content": architecture or "# architecture\n"},
        {"path": "docs/test-plan.md", "content": test_plan or "# test plan unavailable\n"},
        {"path": "decisions.md", "content": _decisions_summary(run)},
    ]
    # Never write a markdown test plan into an executable Python test path.
    # Historical runs without test_code simply omit the generated test file.
    if test_code:
        files.insert(1, {"path": "tests/test_main.py", "content": test_code})
    return files


def _artifact_manifest(files: list[dict[str, str]]) -> list[dict[str, str]]:
    """Return a stable content-addressed manifest for review/delivery equality."""
    return [
        {
            "path": item["path"],
...
async def stage_deliver(run: RunState) -> AsyncIterator[StageEvent]:
    ...
    if run.contains_synthetic_output:
        yield _ev(
            run, Stage.DELIVER, "failed",
            "Delivery blocked: run contains synthetic provider output",
            pr_url=None,
            delivery_status="blocked_synthetic",
            delivery_reason="synthetic output is allowed only for demo/test evaluation",
            synthetic_stages=list(run.synthetic_stages),
        )
        return

    if not run.reviewed_artifact_manifest:
        yield _ev(
            run, Stage.DELIVER, "failed",
            "Delivery blocked: reviewed artifact manifest is missing",
            pr_url=None,
            delivery_status="blocked_unreviewed",
            delivery_reason="Review Scan must persist the exact artifact manifest before delivery",
        )
        return

    yield _ev(run, Stage.DELIVER, "started", "Preparing delivery PR")
    branch = f"agentic/{run.run_id[:8]}"
    files = _delivery_files(run)
    manifest = _artifact_manifest(files)
    if run.reviewed_artifact_manifest and manifest != run.reviewed_artifact_manifest:
        yield _ev(
            run, Stage.DELIVER, "failed",
            "Delivery blocked: artifact bytes differ from reviewed manifest",
            pr_url=None,
            delivery_status="blocked_artifact_drift",
            reviewed_manifest=run.reviewed_artifact_manifest,
